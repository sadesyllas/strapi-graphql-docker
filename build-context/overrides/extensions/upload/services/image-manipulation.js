...
const toFormat = (buffer, format, options) =>
  sharp(buffer)
    .withMetadata()
...
const generateResponsiveFormats = async file => {
  ...
  const extraFormats =
    await Promise.all(['webp', 'avif'].map(format => generateExtraFormat(format, file)));

  return [...resized, ...extraFormats];
};
...
const generateExtraFormat = async (key, file) => {
  const newBuff = await toFormat(file.buffer, key);
  ...
  if (newBuff) {
    ...
    return {
      key,
      file: {
        name: `${key}_${file.name.replace(new RegExp(`${file.ext}$`), ext)}`,
        hash: `${key}_${file.hash}`,
        ext,
        mime: `image/${key}`,
        width,
